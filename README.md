...
A Cycom-branded Odoo 19 distribution with a custom Next.js front-end.
The Next.js UI owns the design language; Odoo provides the business logic.

## Layout

| Path | What it is |
|---|---|
| `cycom-erp/` | Next.js 16 front-end (React 19, Tailwind 4). Owns the visual design. Talks to Odoo via JSON-RPC through `/api/odoo/*` route handlers. |
| `cycom-platform/` | Odoo 19 distribution: `docker-compose.yml` + 75 Cycom custom modules + `cycom_theme` (backend branding) + `cycom_core` (cross-cutting extensions) + clone-helper for the upstream Odoo source. |
...
The 10 doctrine wizards live under `cycom-erp/app/setup/*` and orchestrate Odoo via `cycom-erp/app/api/odoo/setup/*`:
...
```bash
# 1) Bring up Odoo + Postgres
cd cycom-platform
bash scripts/clone-odoo.sh        # one-off: clones the full Odoo 19 source (~1.4 GB)
cp .env.example .env
docker compose up -d
...
- Odoo Community (in `cycom-platform/odoo-source/`, not vendored in this repo): LGPLv3.
