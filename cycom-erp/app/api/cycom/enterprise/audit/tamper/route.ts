...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const upstream = await fetch(`${CYCOM_BACKEND_URL}/api/enterprise/audit/tamper?log_id=${log_id}&action=${encodeURIComponent(action)}`, {
      method: 'POST',
      headers: {
    ...
    if (!upstream.ok) {
      return NextResponse.json({ error: payload.detail || 'Tamper simulation failed' }, { status: upstream.status });
    }
