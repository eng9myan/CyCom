...
export async function POST(req: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${CYCOM_BACKEND_URL}/api/enterprise/audit/verify`, {
      method: 'POST',
      headers: {
    ...
    if (!upstream.ok) {
      return NextResponse.json({ error: payload.detail || 'Audit verification failed' }, { status: upstream.status });
    }
