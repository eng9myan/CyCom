 ...
 *
 * 1. Resolve the requested l10n_<cc> Odoo module
 ...
 *
 * Note: install is synchronous from Odoo's perspective but can take a while. Next.js route handlers
...
async function rpc<T = unknown>(
  ...
  if (data.error) {
    throw new Error(data.error.data?.message || data.error.message || `Odoo error on ${model}.${method}`);
  }
  ...
  try {
    ...
    if (!rows.length) {
      return NextResponse.json(
        {
          ok: false,
          error: `Odoo does not expose a module named "${payload.l10nModule}". Click Update Apps List in Odoo, or pick a different localization.`,
        },
        { status: 404 },
      );
    }
