...
import { installModules, odooRpc, setParam } from '@/lib/setup/serverHelpers';
  ...
  try {
    await installModules(req, [
      ...
      ...(p.enableBiometricZk ? [
        ...
        { name: 'odoo_biometric_attendance' },
    ...
    for (const name of p.departments) {
      ...
      const existing = await odooRpc<Array<{ id: number }>>(
      ...
      try {
        const id = await odooRpc<number>(req, 'hr.department', 'create', [{ name: name.trim() }]);
