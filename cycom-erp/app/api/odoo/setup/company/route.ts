 ...
 *
 * Takes the business-language wizard payload and translates it into a sequence of Odoo writes:
 ...
 * because Odoo doesn't support cross-call transactions over JSON-RPC, but we tell the user
...
// Tiny helper: invoke odooCallKw and unwrap result/error.
async function rpc<T = unknown>(
  ...
  if (data.error) {
    const msg = data.error.data?.message || data.error.message || `Odoo error on ${model}.${method}`;
    throw new Error(msg);
  }
...
async function findCurrencyId(req: NextRequest, name: string): Promise<number> {
  ...
  if (!rows.length) throw new Error(`Currency not found in Odoo: ${name}`);
  if (!rows[0].active) {
